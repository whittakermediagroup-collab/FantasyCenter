...
function scoreCandidate(p, ctx) {
  ...
  return {
    ...
    durabilityNote: dur.note, usageNote: usage.note, handcuffNote, isHandcuff: !!starterName,
  };
}
...
export default function DraftCommand() {
  ...
  // Pull EVERY true rookie (ESPN experience.years === 0) at an offensive
  // skill position across all 32 active rosters (data/rookies.json, built
  // by scripts/update-rookies.mjs) and add them as new players - not just
  // the handful of headline names I happened to hand-research. Appends
  // new player objects rather than merging fields, since these players
  // don't exist in the static pool at all. IDs continue past the static
  // pool's range so they never collide. Any player already in the static
  // pool (e.g. Jeremiyah Love, hand-calibrated) is skipped server-side by
  // the script itself, so this never overwrites a better projection.
  ...
  return (
    <div className="w-full draft-command-root" style={{ background: "radial-gradient(ellipse 1200px 800px at 50% -10%, #14261C 0%, var(--bg) 55%)", color: "var(--text)", minHeight: "100%" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {/* RECOMMENDATION HERO */}
        {topPick && (
          <div key={topPick.player.id} className="border mb-6 glow hero-enter lift overflow-hidden flex" style={{ background: "linear-gradient(135deg, var(--surface) 0%, var(--surface2) 100%)", borderColor: "color-mix(in srgb, var(--turf-bright) 30%, transparent)", borderRadius: "var(--r-hero)" }}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {DURABILITY[topPick.player.name] && (
                    <span className="text-xs font-bold mono px-2 py-1 rounded" style={{ color: "var(--warning)", background: "color-mix(in srgb, var(--warning) 12%, transparent)", border: "1px solid color-mix(in srgb, var(--warning) 40%, transparent)" }} title={DURABILITY[topPick.player.name].note}>
                      ⛑ {DURABILITY[topPick.player.name].missed} GM MISSED (3YR)
                    </span>
                  )}
                  {typeof topPick.player.snapPct === "number" && (
                    <span className="text-xs font-bold mono px-2 py-1 rounded" style={{ color: "var(--info)", background: "color-mix(in srgb, var(--info) 12%, transparent)", border: "1px solid color-mix(in srgb, var(--info) 40%, transparent)" }} title="Snap share and target share from the final 2025 regular season (nflverse) - real usage, not this preseason.">
                      {topPick.player.snapPct}% SNAPS{typeof topPick.player.targetSharePct === "number" ? ` · ${topPick.player.targetSharePct}% TGT` : ""} ('25)
                    </span>
                  )}
            ...
            <div className="p-4 border-b flex flex-wrap items-center gap-3" style={{ borderColor: "var(--border)" }}>
              ...
              <div className="flex items-center gap-1 flex-wrap">
                ...
                <button onClick={() => setPosFilter("ROOKIES")}
                  ...
                  title="Browse the auto-discovered rookie class directly - they sink to the bottom of the default sort by design, so this filter is the fast way to see them all.">
                  ROOKIES
                </button>
              </div>
              <select value={tableSortBy} onChange={(e) => setTableSortBy(e.target.value)}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {sortedForTable.slice(0, 80).map((r, i) => {
                    ...
                    return (
                      <tr key={r.player.id} className="border-t" style={{ borderColor: "var(--border)", background: isTop ? "color-mix(in srgb, var(--turf-bright) 6%, transparent)" : "transparent" }}>
                        ...
                        <td className="px-2 py-2 font-medium">
                          <div className="flex items-center gap-1.5">
                            ...
                            {r.player.isRookiePlaceholder && (
                              <span className="text-xs font-medium px-1.5 py-0.5 rounded" style={{ color: "var(--pos-k)", background: "color-mix(in srgb, var(--pos-k) 14%, transparent)", border: "1px solid color-mix(in srgb, var(--pos-k) 40%, transparent)" }} title="Auto-discovered true rookie (ESPN roster data) - points are a flat conservative placeholder by position, not a researched projection.">
                                ROOKIE (est.)
                              </span>
                            )}
