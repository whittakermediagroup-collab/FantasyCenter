...
async function getRosterNames(teamId) {
  ...
  for (const group of groups) {
    for (const athlete of group.items || []) {
      if (athlete.fullName) names.push(athlete.fullName);
    }
  }
  return names;
}
...
async function main() {
  ...
  const rosterLists = await mapWithConcurrency(teams, CONCURRENCY, async (team) => {
    const names = await getRosterNames(team.id);
    return { abbr: team.abbr, names };
  });

  // Build normalized-name -> current team map from every roster we found.
  const liveTeamByName = new Map();
  for (const roster of rosterLists) {
    if (!roster) continue;
    for (const name of roster.names) {
      liveTeamByName.set(normalizeName(name), roster.abbr);
    }
  }
  ...
  console.log(`Wrote current teams for ${matched} players (${movedOrCut} changed from the static pool) to ${OUTPUT_PATH}`);
}
