...
const POS_COLOR = {
  QB: { fg: "#C9A8FF", bg: "rgba(201,168,255,0.12)" },
  ...
  DST:{ fg: "#FB7185", bg: "rgba(251,113,133,0.12)" },
};
...
function InjuryFreshnessBanner({ injuryMeta }) {
  ...
  const color = tone === "fresh" ? "var(--turf-bright)" : tone === "aging" ? "var(--amber)" : "#FB7185";
  const label = !isLive
  ...
  return (
    <div className="rounded-xl border px-4 py-3 mb-6 flex flex-wrap items-center justify-between gap-3" style={{ background: "var(--surface)", borderColor: color + "55" }}>
...
const INJURY_COLOR = { Q: "#FFB020", D: "#FB923C", O: "#FB7185" };
...
export default function DraftCommand() {
  ...
  const clockColor = clockSeconds > 60 ? "var(--turf-bright)" : clockSeconds > 20 ? "var(--amber)" : "#FB7185";
  ...
  return (
    <div style={{ background: "var(--bg)", color: "var(--text)", minHeight: "100%", fontFamily: "'Inter',sans-serif" }} className="w-full">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {/* STATUS BAR */}
        <div className="rounded-xl border px-4 py-3 mb-6 flex flex-wrap items-center justify-between gap-4"
          ...
          <div className="flex items-center gap-4">
            {isMyClock && (
              <div className="flex items-center gap-2 rounded-lg px-3 py-1.5 border" style={{ borderColor: clockColor, background: "var(--surface2)" }}>
                <span className={`mono text-2xl font-bold ${clockSeconds <= 20 ? "pulse" : ""}`} style={{ color: clockColor }}>{clockMin}:{clockSec}</span>
              ...
              return (
                <div key={r.pos} className="rounded-lg px-3 py-2" style={{ background: "var(--surface2)", border: `1px solid var(--border)` }}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-xs mono" style={{ color: critical ? "#FB7185" : "var(--text-dim)" }}>
                      {r.remainingOfTop}/{r.topN}
                    </span>
                  </div>
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "#1E2B25" }}>
                    <div className={`h-full ${critical ? "pulse" : ""}`} style={{ width: `${r.pct * 100}%`, background: critical ? "#FB7185" : c.fg }} />
                  </div>
                </div>
              );
            })}
        ...
        {/* RECOMMENDATION HERO */}
        {topPick && (
          <div className="rounded-2xl border p-5 sm:p-6 mb-6 glow" style={{ background: "linear-gradient(135deg, var(--surface) 0%, var(--surface2) 100%)", borderColor: "rgba(124,227,139,0.3)" }}>
            <div className="flex items-center gap-2 mb-3">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-5">
              <div className="flex-1">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-sm" style={{ color: topPick.player.team === "FA" ? "#FB7185" : "var(--text-dim)", fontWeight: topPick.player.team === "FA" ? 700 : 400 }}>{topPick.player.team}</span>
                  <InjuryBadge injury={topPick.player.injury} />
                  {topPick.meta.goneBeforeNextPick && (
                    <span className="text-xs font-bold mono px-2 py-1 rounded pulse" style={{ color: "#FB7185", background: "rgba(251,113,133,0.12)", border: "1px solid rgba(251,113,133,0.4)" }}>
                  ...
                  {topPick.meta.isHandcuff && (
                    <span className="text-xs font-bold mono px-2 py-1 rounded" style={{ color: "#C9A8FF", background: "rgba(201,168,255,0.12)", border: "1px solid rgba(201,168,255,0.4)" }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
                      ...
                      <tr key={r.player.id} className="border-t" style={{ borderColor: "var(--border)", background: isTop ? "rgba(124,227,139,0.06)" : "transparent" }}>
                        ...
                        <td className="px-2 py-2 font-medium">
                          <div className="flex items-center gap-1.5">
                            {r.player.name}<span className="text-xs" style={{ color: r.player.team === "FA" ? "#FB7185" : "var(--text-dim)", fontWeight: r.player.team === "FA" ? 700 : 400 }}>{r.player.team}</span>
                            <InjuryBadge injury={r.player.injury} />
                            {r.meta.goneBeforeNextPick && (
                              <span className="text-xs font-bold mono px-1.5 py-0.5 rounded" style={{ color: "#FB7185", background: "rgba(251,113,133,0.12)", border: "1px solid rgba(251,113,133,0.4)" }} title="Market ADP has him gone before your next turn">
                                GONE BY YOUR PICK
                              </span>
                            )}
                            ...
                            {r.meta.isHandcuff && (
                              <span className="text-xs font-bold mono px-1.5 py-0.5 rounded" style={{ color: "#C9A8FF", background: "rgba(201,168,255,0.12)", border: "1px solid rgba(201,168,255,0.4)" }} title={r.meta.handcuffNote || `Handcuff to ${HANDCUFFS[r.player.name]}`}>
          ...
          <div className="rounded-xl border p-4 h-fit" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
...
function MyTeamPage({ lineupPlayers, myRoster, slotOptionsFor, currentSlotFor, moveToSlot, grade, starterVBDTotal }) {
      ...
      <div className="rounded-lg border p-3 flex items-center justify-between gap-3" style={{ background: "var(--surface2)", borderColor: "var(--border)" }}>
        <div className="min-w-0">
          ...
          <div className="text-xs mono mt-0.5" style={{ color: "var(--text-dim)" }}>
            {p.team} · {p.pts} pts · <span style={{ color: "var(--amber)" }}>{vbd > 0 ? "+" : ""}{vbd} VBD</span>
          </div>
        </div>
  ...
  return (
    <div>
      <div className="rounded-xl border p-5 mb-6 flex flex-wrap items-center justify-between gap-4" style={{ background: "linear-gradient(135deg, var(--surface) 0%, var(--surface2) 100%)", borderColor: "var(--border)" }}>
        <div>
