...
const RAW = [
  ...
  ["RB","Joe Mixon","HOU",235],["RB","James Conner","ARI",230],["RB","Jaylen Warren","PIT",225],
  ...
  ["RB","Zach Charbonnet","SEA",198],["RB","Isiah Pacheco","KC",195],["RB","David Montgomery","DET",192],
...
function scoreCandidate(p, ctx) {
  const { bucketCounts, round, undraftedByPos, rbCountOnMyTeam, strategy } = ctx;
  ...
  const score = vbd * mult * (1 + scarcity) * dz.mult * injuryMult;
  return { vbd: Math.round(vbd), mult, scarcity, tierCount, score, starterNeeded, flexNeeded, flexEligible, dzNote: dz.note };
}
...
export default function DraftCommand() {
  ...
  const ranked = useMemo(() => {
    const ctx = { bucketCounts, round, undraftedByPos, rbCountOnMyTeam, strategy };
    return undrafted
      .map((p) => ({ player: p, meta: scoreCandidate(p, ctx) }))
      .sort((a, b) => b.meta.score - a.meta.score);
  }, [undrafted, bucketCounts, round, undraftedByPos, rbCountOnMyTeam, strategy]);
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-5">
              <div className="flex-1">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-sm" style={{ color: "var(--text-dim)" }}>{topPick.player.team}</span>
                      ...
                      <tr key={r.player.id} className="border-t" style={{ borderColor: "var(--border)", background: isTop ? "rgba(124,227,139,0.06)" : "transparent" }}>
                        ...
                        <td className="px-2 py-2 font-medium">
                          <div className="flex items-center gap-1.5">
                            {r.player.name}<span className="text-xs" style={{ color: "var(--text-dim)" }}>{r.player.team}</span>
