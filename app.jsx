  ...
  return (
    <div className="w-full draft-command-root" style={{ background: "radial-gradient(ellipse 1200px 800px at 50% -10%, #14261C 0%, var(--bg) 55%)", color: "var(--text)", minHeight: "100%" }}>
      <style>{`
        ...
        .glow{ box-shadow: inset 0 0 0 1px rgba(124,227,139,0.3), 0 6px 16px -8px rgba(10,16,13,0.6); }
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2.5">
                <span className="flex items-center justify-center rounded-lg" style={{ width: 34, height: 34, background: "linear-gradient(155deg, var(--turf) 0%, #1D7A38 100%)", boxShadow: "0 4px 14px -4px rgba(47,168,79,0.55)" }}>
                  <Trophy size={18} color="#06120A" />
                </span>
                <h1 className="display text-3xl sm:text-4xl font-bold tracking-tight" style={{ color: "var(--text)" }}>DRAFT COMMAND</h1>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {/* RECOMMENDATION HERO */}
        {topPick && (
          <div key={topPick.player.id} className="border mb-6 glow hero-enter lift overflow-hidden flex" style={{ background: "linear-gradient(135deg, var(--surface) 0%, var(--surface2) 100%)", borderColor: "rgba(124,227,139,0.3)", borderRadius: "var(--r-hero)" }}>
            <div className="w-1.5 sm:w-2 shrink-0" style={{ background: "linear-gradient(180deg, var(--turf-bright) 0%, var(--turf) 100%)" }} />
            <div className="flex-1 p-5 sm:p-6">
            <span className="inline-flex items-center gap-1.5 text-xs font-bold mono tracking-widest px-2.5 py-1 rounded-full mb-3" style={{ color: "#06120A", background: "var(--turf-bright)" }}>
              <Zap size={12} /> BEST PICK AVAILABLE
            </span>
            <div className="flex flex-col lg:flex-row lg:items-stretch gap-5">
              <div className="flex-1">
                <div className="flex items-center gap-3 flex-wrap">
                  <h3 className="display text-4xl sm:text-5xl font-bold tracking-tight leading-none">{topPick.player.name}</h3>
                ...
                <div className="flex gap-2.5 mt-4 flex-wrap">
                  <div className="rounded-lg px-3.5 py-2" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>
                    ...
                    <div className="display text-xl font-bold leading-tight">{topPick.player.pts}</div>
                  </div>
                  {typeof topPick.player.adp === "number" && (
                    <div className="rounded-lg px-3.5 py-2" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>
                      <div className="text-[10px] mono tracking-wider" style={{ color: "var(--text-dim)" }}>ADP</div>
                      <div className="display text-xl font-bold leading-tight">{topPick.player.adp.toFixed(1)}</div>
                    </div>
                  )}
                  <div className="rounded-lg px-3.5 py-2" style={{ background: "rgba(255,176,32,0.08)", border: "1px solid rgba(255,176,32,0.3)" }}>
                    <div className="text-[10px] mono tracking-wider" style={{ color: "var(--amber)" }}>VBD</div>
                    <div className="display text-xl font-bold leading-tight" style={{ color: "var(--amber)" }}>+{topPick.meta.vbd}</div>
                  </div>
                  <div className="rounded-lg px-3.5 py-2" style={{ background: "rgba(124,227,139,0.08)", border: "1px solid rgba(124,227,139,0.3)" }}>
                    <div className="text-[10px] mono tracking-wider" style={{ color: "var(--turf-bright)" }}>SCORE</div>
                    <div className="display text-xl font-bold leading-tight" style={{ color: "var(--turf-bright)" }}>{Math.round(topPick.meta.score)}</div>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-2 lg:w-52 lg:pl-5 lg:border-l justify-center" style={{ borderColor: "var(--border)" }}>
                <button onClick={() => draftPlayer(topPick.player.id, true)}
                  ...
                  style={{ background: "linear-gradient(155deg, var(--turf-bright) 0%, var(--turf) 100%)", color: "#06120A", boxShadow: "0 4px 14px -4px rgba(124,227,139,0.5)" }}>
                  DRAFT TO MY TEAM
                </button>
  ...
  function FieldToken({ p, slotLabel, top, left }) {
    ...
    return (
      <div className="absolute flex flex-col items-center" style={{ top, left, transform: "translate(-50%, -50%)", width: 92 }}>
        {p ? (
          <>
            <div className="rounded-full flex items-center justify-center display font-bold text-sm relative"
              style={{ width: 52, height: 52, background: c.bg, color: c.fg, border: `2px solid ${c.fg}`, boxShadow: "0 4px 10px -3px rgba(0,0,0,0.5)" }}>
              {initials(p.name)}
              {p.injury && (
    ...
    return (
      <div>
        <div className="relative rounded-xl overflow-hidden border lift" style={{ borderColor: "var(--border)", aspectRatio: "16 / 11", background: "repeating-linear-gradient(180deg, #163420 0px, #163420 44px, #122C1B 44px, #122C1B 88px)" }}>
          {/* end zones */}
          <div className="absolute top-0 left-0 right-0" style={{ height: "9%", background: "rgba(0,0,0,0.35)", borderBottom: "2px solid rgba(255,255,255,0.25)" }} />
          <div className="absolute bottom-0 left-0 right-0" style={{ height: "9%", background: "rgba(0,0,0,0.35)", borderTop: "2px solid rgba(255,255,255,0.25)" }} />
          {/* yard lines */}
          {[20, 35, 50, 65, 80].map((pct) => (
            <div key={pct} className="absolute left-0 right-0" style={{ top: `${pct}%`, height: 1, background: "rgba(255,255,255,0.18)" }} />
          ))}
          <div className="absolute left-0 right-0 flex justify-center" style={{ top: "50%", transform: "translateY(-50%)" }}>
            <div className="rounded-full" style={{ width: 70, height: 70, border: "2px solid rgba(255,255,255,0.15)" }} />
          </div>
              ...
              {[...lineupPlayers.BENCH, ...lineupPlayers.IR].map((p) => {
                ...
                return (
                  <div key={p.id} className="flex items-center gap-2 rounded-full pl-1 pr-3 py-1" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>
                    <span className="rounded-full flex items-center justify-center text-[10px] font-bold display" style={{ width: 24, height: 24, background: c.bg, color: c.fg }}>{initials(p.name)}</span>
      ...
      {myRoster.length === 0 ? (
        ...
        <div className="flex items-center gap-2 mb-4">
          <button onClick={() => setRosterView("field")}
            ...
            style={rosterView === "field" ? { background: "var(--turf)", borderColor: "var(--turf)", color: "#06120A" } : { background: "var(--surface2)", borderColor: "var(--border)", color: "var(--text-dim)" }}>
            <Grid3x3 size={14} /> Field View
          </button>
          <button onClick={() => setRosterView("list")}
            className="flex items-center gap-1.5 text-sm rounded-md px-3 py-1.5 border font-semibold"
            style={rosterView === "list" ? { background: "var(--turf)", borderColor: "var(--turf)", color: "#06120A" } : { background: "var(--surface2)", borderColor: "var(--border)", color: "var(--text-dim)" }}>
