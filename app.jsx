                    ...
                    return (
                      <tr key={r.player.id} className="border-t" style={{ borderColor: "var(--border)", background: isTop ? "color-mix(in srgb, var(--turf-bright) 6%, transparent)" : "transparent" }}>
                        ...
                        <td className="px-2 py-2 text-right mono" style={{ color: "var(--text-dim)" }}>
                          {typeof r.player.adp === "number" ? r.player.adp.toFixed(1) : "-"}
                          {typeof r.player.fpPosRank === "number" && (
                            <span className="ml-1" style={{ color: "var(--info)" }} title="FantasyPros consensus positional rank (ESPN/CBS/RTSports/Fantrax/Sleeper blend) - a second, independent market signal alongside FFC's ADP.">FP{r.player.fpPosRank}</span>
                          )}
                        </td>
