              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {sortedForTable.slice(0, 80).map((r, i) => {
                    ...
                    return (
                      <tr key={r.player.id} onClick={() => setSelectedPlayerId(r.player.id)} className="border-t cursor-pointer" style={{ borderColor: "var(--border)", background: isTop ? "color-mix(in srgb, var(--turf-bright) 6%, transparent)" : "transparent" }}>
      ...
      return (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:p-8" style={{ background: "rgba(10,16,13,0.75)" }} onClick={() => setSelectedPlayerId(null)}>
          <div className="w-full max-w-3xl mt-8" onClick={(e) => e.stopPropagation()}>
            <PlayerCard r={r} teamLogos={teamLogos} round={round} draftPlayer={draftPlayer} onClose={() => setSelectedPlayerId(null)} />
          </div>
        </div>
      );
    })()}
...
function PlayerCard({ r, teamLogos, round, draftPlayer, badge, onClose }) {
  const { player, meta } = r;
  return (
    <div className="border glow lift overflow-hidden flex" style={{ background: "linear-gradient(135deg, var(--surface) 0%, var(--surface2) 100%)", borderColor: "color-mix(in srgb, var(--turf-bright) 30%, transparent)", borderRadius: "var(--r-hero)" }}>
      ...
      <div className="flex-1 p-5 sm:p-6 relative">
        {onClose && (
          <button onClick={onClose} className="absolute top-4 right-4 text-xs mono px-2 py-1 rounded border" style={{ borderColor: "var(--border)", color: "var(--text-dim)" }}>
            Close ✕
          </button>
        )}
        {badge && (
        ...
        <div className="flex flex-col lg:flex-row lg:items-stretch gap-5">
          <div className="flex-1">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <h3 className="display text-4xl sm:text-5xl font-bold tracking-tight leading-none">{player.name}</h3>
